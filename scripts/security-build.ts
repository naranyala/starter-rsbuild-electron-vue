  ...
  private async analyzeSourceCode(): Promise<void> {
    ...
    try {
      ...
      const eslintResult = spawnSync(
        'npx',
        ['eslint', 'src/', '--ext', '.ts,.js,.vue', '--format', 'json'],
        {
          cwd: CONFIG.PROJECT_ROOT,
          encoding: 'utf8',
        }
        ...
        try {
          ...
          for (const result of eslintOutput) {
            if (
              ...
              this.errors.push(
                `Security issue in ${result.filePath}: ${JSON.stringify(result.messages)}`
              );
            }
          }
      ...
      if (evalCheck.length > 0) {
        this.errors.push(
          `Found ${evalCheck.length} potentially dangerous eval() usage(s)`
        );
      }
      ...
      if (dynamicImportCheck.length > 0) {
        this.errors.push(
          `Found ${dynamicImportCheck.length} potentially unsafe dynamic import(s)`
        );
      }
        ...
        this.walkDirectory(dirPath, filePath => {
          if (path.extname(filePath).match(/\.(ts|js|vue)$/)) {
            ...
            if (dangerousPattern.test(content)) {
              results.push(
                `${filePath} contains potentially unsafe dynamic import`
              );
            }
          }
        });
    ...
    try {
      ...
      for (const dir of CONFIG.SOURCE_DIRS) {
        ...
        if (fs.existsSync(dirPath)) {
          this.walkDirectory(dirPath, filePath => {
            if (
              path.extname(filePath).match(/\.(ts|js|vue|json|html|css|env)$/)
            ) {
              const content = fs.readFileSync(filePath, 'utf8');
              ...
              for (let i = 0; i < credentialPatterns.length; i++) {
                ...
                if (matches) {
                  this.errors.push(
                    `Potential secret found in ${filePath}: ${matches[0]}`
                  );
                }
              }
            }
      ...
      if (fs.existsSync(packageJsonPath)) {
        ...
        for (const [depName, depVersion] of Object.entries(dependencies)) {
          try {
                ...
                const licenseType =
                  typeof license === 'string'
                    ...
                    : Array.isArray(license)
                      ? license[0]?.type || license[0]
                      : license?.type || 'UNKNOWN';

                if (
                  CONFIG.BLOCKED_LICENSES.some(blocked =>
                    licenseType.includes(blocked)
                  )
                ) {
                  this.errors.push(
                    `Dependency ${depName} has blocked license: ${licenseType}`
                  );
                } else if (
                  !CONFIG.ALLOWED_LICENSES.some(allowed =>
                    licenseType.includes(allowed)
                  )
                ) {
                  this.warnings.push(
                    `Dependency ${depName} has unknown license: ${licenseType}`
                  );
                }
              }
            }
            ...
            this.warnings.push(
              `Could not check license for ${depName}: ${e.message}`
            );
          }
        }
      }
          ...
          this.walkDirectory(dirPath, filePath => {
            if (path.extname(filePath).match(/\.(ts|js|vue)$/)) {
              ...
              for (const pattern of vulnerablePatterns) {
                if (pattern.test(content)) {
                  this.errors.push(
                    `Potentially vulnerable code pattern found in ${filePath}`
                  );
                }
              }
            }
      ...
      for (const configFile of mainConfigFiles) {
        ...
        if (fs.existsSync(configPath)) {
          ...
          if (configContent.includes('nodeIntegration: true')) {
            this.errors.push(
              `Insecure setting found in ${configFile}: nodeIntegration: true`
            );
          }

          if (!configContent.includes('contextIsolation: true')) {
            this.errors.push(
              `Missing secure setting in ${configFile}: contextIsolation: true`
            );
          }
        }
      }
