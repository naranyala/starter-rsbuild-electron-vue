...
function registerElectronArchitectureHandler() {
  ipcMain.handle('use-case:electron-architecture', async (event, data) => {
    try {
      ...
      console.error('Error in electron-architecture handler:', error);
      return { success: false, error: error.message };
    }
  });
}

module.exports = { registerElectronArchitectureHandler };
