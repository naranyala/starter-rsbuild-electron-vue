...
const ipcHandlers = new Map<string, HandlerInfo>();
...
export function registerListener(
  ...
  options: { errorHandler?: (error: Error, event: Electron.IpcMainEvent, ...args: any[]) => void } = {}
): void {
 ...
 * @param channel - IPC channel name
 */
export function unregisterListener(channel: string): void {
  if (ipcHandlers.has(channel)) {
    const handlerInfo = ipcHandlers.get(channel);
    if (handlerInfo) {
      ipcMain.removeListener(channel, handlerInfo.handler as (event: Electron.IpcMainEvent, ...args: any[]) => void);
    }
    ipcHandlers.delete(channel);
  }
}
 ...
 * @param args - Arguments to send
 */
export function sendToWindowByName(windowName: string, channel: string, ...args: any[]): void {
  const windows = BrowserWindow.getAllWindows();
 ...
 */
export function getRegisteredHandlers(): Map<string, HandlerInfo> {
...
export function registerAppHandlers(options: AppHandlerOptions = {}): void {
  ...
  // Get app path
  registerHandler('app:getPath', (_event, name: any) => app.getPath(name));

  // Set user model ID (Windows)
  registerHandler('app:setUserModelID', (_event, id: string) => {
  ...
  // Set app badge
  registerHandler('app:setBadgeCount', (_event, count: number) => {
...
export class IpcManager {
  private static channels: Map<string, Function> = new Map();
  ...
  static registerHandler(
    ...
    const validatedHandler = async (event: IpcMainInvokeEvent, ...args: any[]) => {
  ...
  static registerOnceHandler(
    ...
    const onceHandler = async (event: IpcMainInvokeEvent, ...args: any[]) => {
...
export class IpcEventManager {
  private static listeners: Map<string, Map<number, Function>> = new Map();
