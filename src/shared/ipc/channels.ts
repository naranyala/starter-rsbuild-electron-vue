...
/**
 * Get all channel strings recursively
 */
export function getAllChannels(): string[] {
  const channels: string[] = [];

  function extractChannels(obj: Record<string, unknown>): void {
    for (const value of Object.values(obj)) {
      if (typeof value === 'object' && value !== null) {
        extractChannels(value as Record<string, unknown>);
  ...
  extractChannels(IPC_CHANNELS as unknown as Record<string, unknown>);
  return channels;
}
