...
export function fileExists(filePath) {
    try {
        ...
        console.error('Error checking file existence:', error instanceof Error ? error.message : String(error));
        return false;
    }
}
/**
 ...
 * @returns True if successful
 */
export function copyPath(sourcePath, destPath) {
    try {
 ...
 */
export function getFileModifiedTime(filePath) {
    try {
        const stats = fs.statSync(filePath);
        return stats.mtime;
    }
 ...
 */
export function watchPath(targetPath, callback, options) {
    try {
        if (options?.recursive !== undefined) {
            ...
            return fs.watch(targetPath, options?.encoding || 'utf8', callback);
        }
    }
    catch (error) {
...
export const AsyncFileOperations = {
     ...
     */
    async copyPath(sourcePath, destPath) {
        try {
