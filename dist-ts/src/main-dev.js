...
function registerIpcHandlers() {
    ...
    ipcMain.handle('dialog:showMessageDialog', async (event, options) => {
        const focusedWindow = BrowserWindow.fromWebContents(event.sender);
        const targetWindow = focusedWindow || BrowserWindow.getFocusedWindow();
        if (!targetWindow) {
            throw new Error('No available window for dialog');
    ...
    // Process handlers
    ipcMain.handle('process:execCommand', async (event, command, options = {}) => {
        return ProcessService_executeCommand(command, options);
    });
    ipcMain.handle('process:spawnProcess', async (event, command, args = [], options = {}) => {
        return {
            pid: null,
            message: 'Process spawning not implemented in this example',
        };
    });
    ipcMain.handle('process:killProcess', async (event, pid) => {
        try {
            process.kill(pid);
            return true;
        }
    ...
    // Menu handlers
    ipcMain.handle('menu:showContextMenu', async (event, template) => {
    ...
    ipcMain.handle('menu:setApplicationMenu', async (event, template) => {
    ...
    ipcMain.handle('shell:openPath', async (event, pathStr) => {
